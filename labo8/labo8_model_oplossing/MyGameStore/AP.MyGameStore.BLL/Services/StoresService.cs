...
namespace AP.MyGameStore.BLL.Services
{
    public class StoresService : IStoresService
    {
        ...
        public PagedResult<Store> GetAll(string? nameFilter = null, SortBy? sortBy = null,int pageNumber = 1, int pageLength = 10)
        {
            ...
            if (pageLength > 20)
                throw new ValidationException("PageLength is limited to 20 items", ValidationException.ErrorCode.PageLengthGreaterThan20);
            if (pageNumber < 1)
                throw new ValidationException("PageNumber cannot be 0 or negative", ValidationException.ErrorCode.PageNumberSmallerThan1);
        ...
        public Store Add(Store store)
        {
            ...
            if (existing != null)
                throw new ValidationException("A store with this name already exists", ValidationException.ErrorCode.NameNotUnique);
        ...
        public void Delete(int id)
        {
            ...
            if (uow.StoresRepository.GetEmployees(id).Count() > 0)
                throw new ValidationException("There are still employees in this store", ValidationException.ErrorCode.StillEmployeesInStore);
        ...
        public Store Update(Store store)
        {
            ...
            if (existing.Name != store.Name)
            {
                ...
                if (exists != null)
                    throw new ValidationException("A store with this name already exists", ValidationException.ErrorCode.StoreNotFound);
            }
